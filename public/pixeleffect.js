let ParticleImageDisplayer = function (tag_id, canvas_el, params) {
    "use strict";
    this.pImageConfig = {
        particles: {
            array: [],
    ...
    pImg.functions.image.resize = function () {
        if (pImg.image.aspect_ratio < pImg.canvas.aspect_ratio) { // canvas height constrains image size
        ...
        } else { // canvas width constrains image size
    ...
    pImg.functions.image.init = function () {
        ...
        pImg.image.obj.addEventListener("load", function () { // get aspect ratio (only have to compute once on initial load)
    ...
    pImg.functions.particles.updateParticles = function () {
        for (let p of pImg.particles.array) {
            if (pImg.particles.movement.restless.enabled && p.restlessness.on_curr_frame) { // if restless activity is enabled & particle is in restless mode, animate some random movement
                pImg.functions.particles.jitterParticle(p);
            } else { // otherwise, update position with approach to destination
    ...
    /*
      ...
      ========================================
      */
    pImg.functions.interactivity.repulseParticle = function (p, args) { // compute distance to mouse
...
/*
    ...
    ========================================
    */
Object.deepExtend = function (destination, source) { // credit: https://andrewdupont.net/2009/08/28/deep-extending-objects-in-javascript/
...
window.particleImageDisplay = function (tag_id) { // get target element by ID, check for existing canvases
    ...
    if (canvas != null) {
        ...
        // This chunk will be able to import Object parameters
        ...
        //     if (xhr.readyState === 4 && xhr.status === 200) { // parse parameters & launch display
        //         const params = JSON.parse(xhr.responseText);
        //         console.log(params)
        //         pImgDom.push(new ParticleImageDisplayer(tag_id, canvas, params));
        //     } else {
        //         console.log(`failed to load params.json. XMLHTTPRequest status: ${
        //             xhr.statusText
        //         }`);
        //     }
        // };
